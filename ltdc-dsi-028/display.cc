...
// =============================================================================
// Pipeline:  framebuffer (DDR) -> LTDC -> DSI host -> D-PHY -> ST7701S panel.
...
// the image. See panel_st7701s.hh.
...
uint32_t display_clocks_setup()
{
	...
	// Target: the largest PLL4/N not above Panel::PixelClkMaxHz (~21 MHz for
	// 480x480 @ 60 Hz; PLL4 = 1200 MHz -> /58 = 20.7 MHz).
	...
	if (pll4 >= Panel::PixelClkMaxHz) {
		uint32_t findiv = (pll4 + Panel::PixelClkMaxHz - 1) / Panel::PixelClkMaxHz; // ceil -> pix <= max
		if (findiv < 1)
			findiv = 1;
		if (findiv > 64)
			findiv = 64;
...
{
	// ST7701S RESX is active-low: pulse >= 10 us (table 9, TRW), then wait for
	// the reset-cancel/NVM load (TRT, 5 ms) before the first command. 120 ms
	// covers the worst case (reset while in sleep-out blanks for up to 120 ms).
...
void display_video_restart(uint32_t pixel_clk, const DsiVideoOpts &opts)
{
	...
	dsi_video_stop();
	dsi_command_mode();
	dsi_video_start(pixel_clk, opts);
	udelay(1000); // host + wrapper up and waiting for DPI VSYNC
	ltdc_enable(true);
}
...
bool display_init(uint32_t first_fb, uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint32_t bg_argb)
{
	...
	display_video_restart(pixel_clk, DsiVideoOpts{});
