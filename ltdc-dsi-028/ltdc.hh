...
// Gate the scan-out (GCR.LTDCEN). Disable BEFORE stopping/restarting the DSI
// stream and re-enable after: the LTDC feeds the DSI's DPI input continuously,
// and stopping the host mid-line leaves a partial pixel in its DPI FIFO that
// offsets the whole restarted stream by 1-2 bytes (colors rotate, image wraps).
